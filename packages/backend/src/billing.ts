...
/**
 * Minimal Stripe surface our routes use. Tests inject a stub matching this
 * shape; production binds to a real `Stripe` instance. The shape stays narrow
 * on purpose so the stub doesn't need to grow as the real SDK does.
 */
export type StripeLike = {
  checkout: { sessions: { create: Stripe["checkout"]["sessions"]["create"] } };
  webhooks: { constructEventAsync: Stripe["webhooks"]["constructEventAsync"] };
};

export type BillingDeps = {
  /** Factory for the Postgres client. Override in tests. */
  dbFactory: (databaseUrl: string) => Db;
  /** Factory for the Stripe client. Override in tests. */
  stripeFactory: (env: Env) => StripeLike;
};

const ALLOWED_AMOUNTS_CENTS = new Set([500, 1000, 2500, 5000, 10000]);

const defaultStripeFactory = (env: Env): StripeLike =>
  new Stripe(env.STRIPE_SECRET_KEY, {
    httpClient: Stripe.createFetchHttpClient(),
  });

/**
 * Build the billing Hono app. Production callers can use the default-export
 * `billing`; tests inject stubs to avoid Stripe and Postgres.
 */
export function createBilling(
  deps: BillingDeps = { dbFactory: db, stripeFactory: defaultStripeFactory },
): Hono<{ Bindings: Env }> {
  ...
  app.post("/buy", async (c) => {
    ...
    const stripe = deps.stripeFactory(c.env);
...
/** Production billing app. Tests should call `createBilling({...})` instead. */
